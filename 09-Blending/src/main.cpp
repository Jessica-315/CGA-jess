...
void applicationLoop() {
	...
	while (psi) {
		...
		// Actualiza la posición con las posiciones correctas de los objetos con transparencia
		...
		for (itBlend = blendingDesordenado.begin(); itBlend != blendingDesordenado.end(); itBlend++) {
			float distaciaFrontCamara = glm::distance(camera->getPosition(), itBlend->second);
			blendingOrdenado[distaciaFrontCamara] = std::make_pair(itBlend->first, itBlend->second);
		}

		//Render los objetos con transparencia
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);//Esta es la más utilizada, hay mas ejemplos en classroom
		for (std::map<float, std::pair<std::string, glm::vec3>>::reverse_iterator it = blendingOrdenado.rbegin();//Del mas lejano al mas cercano
			//Primero se tienen que dibujar os objetos sólidos y despues los que tienen trasnparecnia
		...
		glEnable(GL_BLEND);
		//modelText->render("Texto en openGL", -1, 0);//El -1 y 0 son las coordenadas
		modelText->render("Texto en openGL", 0, 0, 1.0, 0.0, 0.0, 24);
		glDisable(GL_BLEND);
