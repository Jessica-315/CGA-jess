...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderParticlesFire.setInt("Pass", 1);//Fase puede ser 1 o 2
	shaderParticlesFire.setInt("ParticleTex", 0);//Textura
	...
	while (psi) {
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			if(it->second.first.compare("aircraft") == 0){
				...
				glActiveTexture(GL_TEXTURE1);//Textura de una dimensión
				glBindTexture(GL_TEXTURE_1D, texId);
				glEnable(GL_RASTERIZER_DISCARD);
				glBindTransformFeedback(GL_TRANSFORM_FEEDBACK, feedback[drawBuf]);
				glBeginTransformFeedback(GL_POINTS);
				glBindVertexArray(particleArray[1-drawBuf]);//Activamos buffer
