...
float disMaxBala = 7.0f; // Distancia máxima de la bala
...
//std::vector<glm::vec3> transladaEnemy[NUMERO_ENEMIGOS];
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Escalamiento de Zombies.
	for (int i = 0; i < NUMERO_ENEMIGOS; i++) {
		escalaEnemy[i] = 0.00015f;
		transladaEnemy[i] = 0.0f;
	}
...
bool processInput(bool continueApplication) {
	...
	posturaJugadorX = 0.0f;
	posturaJugadorY = 0.0f;
	posturaJugadorZ = 0.0f;
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
		...
		posturaJugadorX = 0.2f;
		posturaJugadorY = 0.21f;
		posturaJugadorZ = 0.5f;
	}
...
void applicationLoop() {
	...
	while (psi) {
		...
		*******************************************/
		...
		//vectorMatrixRayBala vectorVarRayBala

		//glm::vec3 rayDirectionBala;
		//glm::vec3 origenBala; //Punto inicial
		//glm::vec3 targetRayBala;
...
void renderScene(bool renderParticles) {
	...
	*/
	...
	for (size_t i = 0; i < enemyCollection.size(); i++)
	{
		...
		enemyCollection[i]->ModelMatrix = glm::translate(enemyCollection[i]->ModelMatrix, glm::vec3(0.0f, transladaEnemy[i], 0.0f));
		...
		enemyCollection[i]->SetScale(glm::vec3(escalaEnemy[i], escalaEnemy[i], escalaEnemy[i]));
