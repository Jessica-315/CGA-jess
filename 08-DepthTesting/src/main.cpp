...
//Shader para dibujar el buffer de profuncidad
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	glEnable(GL_DEPTH_TEST);//Línea para activar el buffer de profundidad
	...
	//Se reemplazó todo el shaderMulLighting por shaderDepthTesting
	modelRock.loadModel("../models/rock/rock.obj");
	modelRock.setShader(&shaderDepthTesting);//modelRock.setShader(&shaderMulLighting);
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderTerrain.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		//Settea la matriz de vista y projection al shader del buffer de profundidad
